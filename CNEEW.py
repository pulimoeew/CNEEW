...
def timer():
    while True:
        try:
            info_text.setText(
                f"中国地震局  {get_bjt().strftime('%H:%M:%S')}  中国地震预警网"
            )
        except:
            error_report()
            continue
                ...
                while True:
                    ...
                    if cenceew_json["type"] != "heartbeat":
                        ...
                        if cnshindo >= 1.0 and cnshindo < 2.0:
                            ...
                            tips_text.setText(
                                f"注意：本地烈度{cnshindo:.1f}，有强烈震感，请紧急避险"
                            )
                            message = f"{eqtime} {location}发生M{magnitude}地震，最大预估烈度{maxshindo}度，本地预估烈度{cnshindo:.1f}度。有强烈震感，请紧急避险！"
                        else:
                            tips_text.setText(
...
if __name__ == "__main__":
    ...
    try:
        ...
        window = MainWindow()
        window.setWindowTitle(f"中国地震预警(CNEEW) v{version}")
        ...
        thread2 = Thread(target=asyncio.run, args=(cneew(window),), daemon=True)
        thread1.start()
        thread2.start()
        app.exec()
    except:
